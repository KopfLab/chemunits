...
#' @export
convert_to_default_units <- function(x, defaults = c("mol", "M", "L", "1")) {
  # safety check
  stopifnot(
  ...
  if (rlang::is_empty(defaults)) return(x)
  ...
  if (sum(valid_units) == 0L) return(x) # no default unit for this quantity
  if (sum(valid_units) > 1L) { # more than one default unit
    sprintf("more than one valid default unit for '%s' - will use '%s' and ignore '%s'",
