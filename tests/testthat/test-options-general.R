...
test_that("functionality works", {
  ...
  expect_equal(opt_c <- define_pkg_option(42, is_integerish), 
               list(default = 42, check_fn_quo = quo(is_integerish)))
  .options <- list(a = opt_a, b = opt_b, c = opt_c)
  
  # set/get_pkg_option
  opts <- options(); on.exit(options(opts))
  ...
  old <- expect_visible(pkg_options("pkg", .options), 
  ...
  expect_equal(options("pkg.a", "pkg.b", "pkg.c"), 
               list(pkg.a = NULL, pkg.b = NULL, pkg.c = NULL))
  expect_equal(pkg_options("pkg", .options), 
               list(a = NULL, b = "default", c = 42))
})
